...
- **Do the work, don't assign homework.** Read the external source (blog post, changelog, advisory) to understand what changed. If the change affects a specific API, config option, or behavior — and you can't already tell from the provider map whether the project is affected — then locate the relevant code (grep for the specific function, flag, or import) and read just that portion to confirm. Don't read broadly; zero in on the exact usage that matters. Never write "check if you're affected" when you can check. Never recommend a migration path you haven't read. The user is reading this briefing to save time, not to get a to-do list.
