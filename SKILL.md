...
- **Do the work, don't assign homework.** Before writing an impact statement or recommendation, do two things: (1) read the external source thoroughly — the blog post, changelog, migration guide, or advisory — so you understand exactly what changed, and (2) read the relevant code in the project — the actual files that use the affected provider, API, or feature — so you understand how the project uses it today. Only then can you write a specific impact statement grounded in both sides. Never write "check if you're affected" when you can check. Never recommend a migration path you haven't read. The user is reading this briefing to save time, not to get a to-do list.
